...
function enemyFor(floor: number, index: number): Enemy {
  ...
  return {
    ...
    kind,
    x,
    y: boss ? floorY - 210 : kind === 'drone' ? floorY - 118 : floorY - 58,
    width: boss ? 180 : kind === 'drone' ? 50 : 46,
    height: boss ? 210 : kind === 'drone' ? 38 : 58,
...
export function createLevel(floor: number, hp = 100, carry: LevelCarry = {}): PlatformGameState {
  ...
  return {
    ...
    bossClawTimer: bossFloor ? 1.2 : 0,
    bossClawX: 620,
    bossLightningWarning: bossFloor ? 10 : 0,
